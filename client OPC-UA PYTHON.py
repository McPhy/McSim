...
# Test_node = client.get_node(df['Nodes'][23])
...
for i in range(len(df['Tag Name'])):
        Nodes.append(client.get_node(df['Nodes'][i]))

        if df['DataType'][i] == 'Bool' :
                dv = ua.DataValue(ua.Variant(1, ua.VariantType.Boolean))
                dv.ServerTimestamp = None
                dv.SourceTimestamp = None
                Nodes[i].set_value(dv)
                #print(str(df['Nodes'][i]))

        # elif df['DataType'][i] == 'Int' :
        #         dv = ua.DataValue(ua.Variant(2, ua.VariantType.Int32))
        #         dv.ServerTimestamp = None
        #         dv.SourceTimestamp = None
        #         Nodes[i-1].set_value(dv)

        elif df['DataType'][i] == 'Real' :
                dv = ua.DataValue(ua.Variant(4.5, ua.VariantType.Float))
                dv.ServerTimestamp = None
                dv.SourceTimestamp = None
                Nodes[i].set_value(dv)


        print(i, Nodes[i], "==========>" , Nodes[i].get_value())
...
#         print(Nodes[i],"==========>", Nodes[i].get_value())
